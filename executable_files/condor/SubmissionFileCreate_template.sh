...
echo "Transfer_Input_Files = `pwd`/run_$1.sh" >> $SubmissionFile
...
while IFS= read -r line; do
   ...
   echo "Output    = `pwd`/$3/out/job_$Count.out" >> $SubmissionFile
   echo "Error     = `pwd`/$3/err/job_$Count.err" >> $SubmissionFile
   echo "Log       = `pwd`/$3/log/job_$Count.log" >> $SubmissionFile
