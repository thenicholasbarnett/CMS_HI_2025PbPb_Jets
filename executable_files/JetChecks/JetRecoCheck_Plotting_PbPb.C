...
void JetRecoCheck_Plotting_PbPb(){

    // getting rid of legends in hists in root file
    gStyle->SetOptStat(0);

    // reading input file
    TString input0 = "JetChecks_run399499_RP20s_11_19_2025.root";
    TString input1 = "JetChecks_run399499_RP20s_11_19_2025.root";
    ...
    TH2F *hjtpfFphi_eta[2][nfrac][netabins];
    ...
    TH2F *hjtpfFphi_hibin[2][nfrac][netabins][nhiBin];
    ...
    for(unsigned int b=0; b<netabins; b++){
        ...
        for(int hj=0; hj<nfrac; hj++){
            ...
            TString htitle8 = "hjt"+sfrac[hj]+"phi"+htitles_byeta[b];
            hjtpfFphi_eta[0][hj][b] = (TH1F*) fi0->Get(htitle8);
            hjtpfFphi_eta[1][hj][b] = (TH1F*) fi1->Get(htitle8);
            if(!hjtpfFphi_eta[0][hj][b]){cerr<<"ERROR: Could not find histogram '"<< htitle8<<"' in files"<<endl;}
        ...
        for(unsigned int hb=0; hb<nhiBin; hb++){
            ...
            for(int hj=0; hj<nfrac; hj++){
                ...
                TString htitle9 = "hjt"+sfrac[hj]+"phi"+htitles_byeta[b]+htitles_byhibin[hb];
                hjtpfFphi_hibin[0][hj][b][hb] = (TH1F*) fi0->Get(htitle9);
                hjtpfFphi_hibin[1][hj][b][hb] = (TH1F*) fi1->Get(htitle9);
                if(!hjtpfFphi_hibin[0][hj][b][hb]){cerr<<"ERROR: Could not find histogram '"<< htitle9<<"' in files"<<endl;}
    ...
    for(int i=0; i<2; i++){
        ...
        for(unsigned int b=0; b<netabins; b++){
            ...
            for(int hj=0; hj<nfrac; hj++){
                Double_t a9 = hjtpfFphi_eta[i][hj][b]->Integral();
                hjtpfFphi_eta[i][hj][b]->Scale(1.0/a9);
            ...
            for(unsigned int hb=0; hb<nhiBin; hb++){
                ...
                for(int hj=0; hj<nfrac; hj++){
                    Double_t a8 = hjtpfFphi_hibin[i][hj][b][hb]->Integral();
                    hjtpfFphi_hibin[i][hj][b][hb]->Scale(1.0/a8);
    ...
    for(unsigned int b=0; b<netabins; b++){
        ...
        // PF fractions & multiplicities
        for(int hj=0; hj<nfrac; hj++){
            if(flag_ep==1){save_2d_byeta(hjtpfFphi_eta, hj, b, htitles_frac_xaxis[hj], htitles_mom_xaxis[3], "Inclusive", "jet_"+htitles_frac_name[hj]+"_phi"+sEtaBin, path_ep);}
        ...
        for(unsigned int hb=0; hb<nhiBin; hb++){
            ...
            for(int hj=0; hj<nfrac; hj++){
                if(flag_ep==1){save_2d_byhibin(hjtpfFphi_hibin, hj, b, hb, htitles_frac_xaxis[hj], htitles_mom_xaxis[3], shiBins[hb], "jet_"+htitles_frac_name[hj]+"_phi"+sEtaBin+shiBin, path_ep);}
