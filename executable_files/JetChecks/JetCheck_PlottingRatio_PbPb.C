...
void JetCheck_PlottingRatio_PbPb(TString input_num, TString input_denom, int run_num){
    ...
    for(int i=0; i<2; i++){
        ...
        for(unsigned int p=0; p<nptcuts; p++){
            ...
            for(unsigned int b=0; b<netabins; b++){
                // momenta 
                for(int hj=0; hj<nmom; hj++){normalize(hjetmom_eta[i][hj][p][b]);}
                // eta phi map
                normalize(hjtetaphi_eta[i][p][b]);
                // fractions and multiplicities
                for(int hj=0; hj<nfrac; hj++){normalize(hjtPfF_eta[i][hj][p][b]);}
                // by hiBin
                for(unsigned int hb=0; hb<nhiBin; hb++){
                    for(int hj=0; hj<nmom; hj++){normalize(hjetmom_hibin[i][hj][p][b][hb]);}
                    normalize(hjtetaphi_hibin[i][p][b][hb]);
                    for(int hj=0; hj<nfrac; hj++){normalize(hjtPfF_hibin[i][hj][p][b][hb]);}
                }
            }
        }
    }
    ...
    for(unsigned int p=0; p<nptcuts; p++){
        for(unsigned int b=0; b<netabins; b++){
            if(b==1){continue;}
            for(int hj=0; hj<nmom; hj++){if(flag_mom==1){save_h1f_1(hjetmom_eta_ratio[hj][p][b], htitles_mom_xaxis[hj], "", htitles_mom_name[hj]+htitles_byptcut[p]+htitles_byeta[b], run_num, path_mom);}}
            if(flag_ep==1){save_2df(hjtetaphi_eta_ratio[p][b], p, htitles_mom_xaxis[2], htitles_mom_xaxis[3], "Inclusive", "jet_eta_phi"+htitles_byptcut[p]+htitles_byeta[b], run_num, path_ep);}
            for(int hj=0; hj<nfrac; hj++){if(flag_pf_frac==1){save_h1f_1(hjtPfF_eta_ratio[hj][p][b], htitles_frac_xaxis[hj], "", htitles_frac_name[hj]+htitles_byptcut[p]+htitles_byeta[b], run_num, path_frac);}}
            for(unsigned int hb=0; hb<nhiBin; hb++){
                for(int hj=0; hj<nmom; hj++){if(flag_mom==1){save_h1f_1(hjetmom_hibin_ratio[hj][p][b][hb], htitles_mom_xaxis[hj], "", htitles_mom_name[hj]+htitles_byptcut[p]+htitles_byeta[b]+htitles_byhibin[hb], run_num, path_mom);}}
                if(flag_ep==1){save_2df(hjtetaphi_hibin_ratio[p][b][hb], p, htitles_mom_xaxis[2], htitles_mom_xaxis[3], shiBins[hb], "jet_eta_phi"+htitles_byptcut[p]+htitles_byeta[b]+htitles_byhibin[hb], run_num, path_ep);}
                for(int hj=0; hj<nfrac; hj++){if(flag_pf_frac==1){save_h1f_1(hjtPfF_hibin_ratio[hj][p][b][hb], htitles_frac_xaxis[hj], "", htitles_frac_name[hj]+htitles_byptcut[p]+htitles_byeta[b]+htitles_byhibin[hb], run_num, path_frac);}}
            }
            for(int hj=0; hj<nmom; hj++){if(flag_mom_ovr==1){save_mom_overlay(hjetmom_hibin_ratio, p, b, hj, htitles_mom_xaxis[hj], "", htitles_mom_name[hj]+htitles_byptcut[p]+htitles_byeta[b]+"_overlay", run_num, path_mom);}}
            for(int hj=0; hj<nfrac; hj++){if(flag_pf_frac_ovr==1){save_pf_overlay(hjtPfF_hibin_ratio, p, b, hj, htitles_frac_xaxis[hj], "", htitles_frac_name[hj]+htitles_byptcut[p]+htitles_byeta[b]+"_overlay", run_num, path_frac);}}
        }
    }
    fi0->Close();
    fi1->Close();
