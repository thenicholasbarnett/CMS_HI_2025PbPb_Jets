...
#include "/afs/cern.ch/user/n/nbarnett/public/header_files/JetCheck_2024PbPb_MC_binning.h"
// #include "/afs/cern.ch/user/n/nbarnett/public/header_files/JetCheck_2025PbPb_MC_binning.h"

void JetRecoCheck_PbPb_MC_lxplus_v0(TString input_file_list, TString output){
// void JetRecoCheck_PbPb_MC_lxplus_v1(TString input_file_list="/afs/cern.ch/user/n/nbarnett/public/txt_files/forest_locations_2025/forests_2024PbPb_DijetMC.txt",TString output="/eos/cms/store/group/phys_heavyions/nbarnett/temp/11_4_2025/JetRecoCheck_2024PbPbMC_v0_11_4_2025.root"){
    ...
    /// for triggers by hibin
    TH1F *hjetmom_hibin_trig[nmom][netabins][nTrigs][nhiBin];
    TH2F *hjtetaphi_hibin_trig[netabins][nTrigs][nhiBin];
    TH1F *hjtPfF_hibin_trig[nfrac][netabins][nTrigs][nhiBin];
    TH1F *hjtPfM_hibin_trig[nmult][netabins][nTrigs][nhiBin];
    ...
    // // making array for names of hists
    // const int nhists = 16;
    // TString shists[nhists];
    // int ihist = 0;
    ...
    for(unsigned int b=0; b<netabins; b++){
        ...
        for(unsigned int tr=0; tr<nTrigs; tr++){
            ...
            for(int hj=0; hj<nmom; hj++){
                TString htitle8 = smom[hj]+sEtaBin+sTrigs_short[tr];
                hjetmom_trig[hj][b][tr] = new TH1F(htitle8, htitle8, nmombins[hj], nmomlo[hj], nmomhi[hj]);
            }
            
            TString htitle9 = "hjtetaphi"+sEtaBin+sTrigs_short[tr];
            hjtetaphi_trig[b][tr] = new TH2F(htitle9, htitle9, nhetabins, etalims[0], etalims[1], nphibins, philims[0], philims[1]);

            for(int hj=0; hj<nfrac; hj++){

                TString htitle10 = sfrac[hj]+sEtaBin+sTrigs_short[tr];
                hjtPfF_trig[hj][b][tr] = new TH1F(htitle10, htitle10, npfbins, pflims[0], pflims[1]);

                TString htitle11 = smult[hj]+sEtaBin+sTrigs_short[tr];
                hjtPfM_trig[hj][b][tr] = new TH1F(htitle11, htitle11, npfbins, pfmlims[0], pfmlims[1]);
            }
            ...
            for(unsigned int hb=0; hb<nhiBin; hb++){
                ...
                for(int hj=0; hj<nmom; hj++){
                    TString htitle12 = smom[hj]+sEtaBin+sTrigs_short[tr]+shiBin;
                    hjetmom_hibin_trig[hj][b][tr][hb] = new TH1F(htitle12, htitle12, nmombins[hj], nmomlo[hj], nmomhi[hj]);
                }

                TString htitle13 = "hjtetaphi"+sEtaBin+sTrigs_short[tr]+shiBin;
                hjtetaphi_hibin_trig[b][tr][hb] = new TH2F(htitle13, htitle13, nhetabins, etalims[0], etalims[1], nphibins, philims[0], philims[1]);

                for(int hj=0; hj<nfrac; hj++){

                    TString htitle14 = sfrac[hj]+sEtaBin+sTrigs_short[tr]+shiBin;
                    hjtPfF_hibin_trig[hj][b][tr][hb] = new TH1F(htitle14, htitle14, npfbins, pflims[0], pflims[1]);

                    TString htitle15 = smult[hj]+sEtaBin+sTrigs_short[tr]+shiBin;
                    hjtPfM_hibin_trig[hj][b][tr][hb] = new TH1F(htitle15, htitle15, npfbins, pfmlims[0], pfmlims[1]);
                }
            }
        }
    }
            ...
            for(unsigned int j=0; j<nref; j++){
                ...
                for(unsigned int b=0; b<netabins; b++){
                    ...
                    for(unsigned int tr=0; tr<nTrigs; tr++){
                        ...
                        // jet momenta
                        hjetmom_trig[0][b][tr]->Fill(rawpt[j], w);
                        hjetmom_trig[1][b][tr]->Fill(jtpt[j], w);
                        hjetmom_trig[2][b][tr]->Fill(jteta[j], w);
                        hjetmom_trig[3][b][tr]->Fill(jtphi[j], w);
                        hjtetaphi_trig[b][tr]->Fill(jteta[j], jtphi[j], w);

                        // jet pf fractions
                        hjtPfF_trig[0][b][tr]->Fill(jtPfCHF[j], w);
                        hjtPfF_trig[1][b][tr]->Fill(jtPfNHF[j], w);
                        hjtPfF_trig[2][b][tr]->Fill(jtPfCEF[j], w);
                        hjtPfF_trig[3][b][tr]->Fill(jtPfNEF[j], w);
                        hjtPfF_trig[4][b][tr]->Fill(jtPfMUF[j], w);

                        // jet pf multiplicities
                        hjtPfM_trig[0][b][tr]->Fill(jtPfCHM[j], w);
                        hjtPfM_trig[1][b][tr]->Fill(jtPfNHM[j], w);
                        hjtPfM_trig[2][b][tr]->Fill(jtPfCEM[j], w);
                        hjtPfM_trig[3][b][tr]->Fill(jtPfNEM[j], w);
                        hjtPfM_trig[4][b][tr]->Fill(jtPfMUM[j], w);
                        ...
                        for(int hb=0; hb<nhiBin; hb++){
                            ...
                            // jet momenta
                            hjetmom_hibin_trig[0][b][tr][hb]->Fill(rawpt[j], w);
                            hjetmom_hibin_trig[1][b][tr][hb]->Fill(jtpt[j], w);
                            hjetmom_hibin_trig[2][b][tr][hb]->Fill(jteta[j], w);
                            hjetmom_hibin_trig[3][b][tr][hb]->Fill(jtphi[j], w);
                            hjtetaphi_hibin_trig[b][tr][hb]->Fill(jteta[j], jtphi[j], w);

                            // jet pf fractions
                            hjtPfF_hibin_trig[0][b][tr][hb]->Fill(jtPfCHF[j], w);
                            hjtPfF_hibin_trig[1][b][tr][hb]->Fill(jtPfNHF[j], w);
                            hjtPfF_hibin_trig[2][b][tr][hb]->Fill(jtPfCEF[j], w);
                            hjtPfF_hibin_trig[3][b][tr][hb]->Fill(jtPfNEF[j], w);
                            hjtPfF_hibin_trig[4][b][tr][hb]->Fill(jtPfMUF[j], w);

                            // jet pf multiplicities
                            hjtPfM_hibin_trig[0][b][tr][hb]->Fill(jtPfCHM[j], w);
                            hjtPfM_hibin_trig[1][b][tr][hb]->Fill(jtPfNHM[j], w);
                            hjtPfM_hibin_trig[2][b][tr][hb]->Fill(jtPfCEM[j], w);
                            hjtPfM_hibin_trig[3][b][tr][hb]->Fill(jtPfNEM[j], w);
                            hjtPfM_hibin_trig[4][b][tr][hb]->Fill(jtPfMUM[j], w);
                        }
                    }
                }
            }
    ...
    for(unsigned int b=0; b<netabins; b++){
        for(unsigned int tr=0; tr<nTrigs; tr++){
            ...
            for(int hb=0; hb<nhiBin; hb++){
                ...
                for(int hj=0; hj<nmom; hj++){hjetmom_hibin_trig[hj][b][tr][hb]->Write();}
                hjtetaphi_hibin_trig[b][tr][hb]->Write();

                for(int hj=0; hj<nfrac; hj++){
                    hjtPfF_hibin_trig[hj][b][tr][hb]->Write();
                    hjtPfM_hibin_trig[hj][b][tr][hb]->Write();
                }
            }
        }
    }
